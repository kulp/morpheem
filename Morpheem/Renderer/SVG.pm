...
sub renderboard
{
	...
	my ($game, $height, $width) = @args{qw(game height width)};
...
sub renderblanks
{
	...
	if (not $game->{_m}{svg}{blanks}) {
		...
		my $g = $svg->group;
		$self->drawmargins($g, x => 6, y => 5, board_width => $blanks_width, board_height => $blanks_height);
		for my $i (0 .. 25) {
			my $x = $i % 6;
			my $y = int($i / 6);
			$self->makeletter($g, x => $x, y => $y, letter => chr(ord('A') + $i), isblank => 1);
		}
	}
